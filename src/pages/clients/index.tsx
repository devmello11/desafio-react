...
export default function ClientesPage() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setLoadingGlobal(null), 600);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('show-loading_cadastro', handleShowLoadingCadastro);
    };
  }, []);
  ...
  const salvarUsuario = async () => {
    ...
    try {
      let response;
      if (usuarioAtual.id) {
        response = await updateClient(usuarioAtual.id, {
      ...
      console.log('Resposta da API ao salvar:', response);
      const dados = await getClients();
      if (Array.isArray(dados)) {
        setUsuarios(dados);
      } else if (Array.isArray(dados.clients)) {
  ...
  const confirmarRemocao = async () => {
    ...
    try {
      const response = await deleteClient(modalConfirm.usuario.id);
      ...
      <>
        {loadingGlobal && (
          <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black bg-opacity-40">
            ...
            <span className="text-2xl font-bold text-indigo-700 drop-shadow-lg">
          ...
          {({ menuAberto }) => {
            return (
              <>
                ...
                <h1 className="text-2xl xs:text-3xl md:text-4xl font-extrabold text-indigo-700 tracking-tight leading-tight drop-shadow-sm break-words">
                      Clientes
                    </h1>
                  </div>
                  <div className="w-full md:w-auto flex justify-center md:justify-end mt-3 md:mt-0">
                    <input
                      type="text"
                      placeholder="Buscar "
                    className="input font-bold w-full max-w-[90vw] sm:max-w-xs bg-gray-50 border-gray-200 text-indigo-900 placeholder-indigo-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 rounded-full px-4 sm:px-5 py-2 shadow-sm transition-all duration-200 text-base sm:text-lg"
                      value={busca}
                      onChange={e => {
              ...
              <div className="flex flex-col gap-6 pt-8 pb-20 min-h-screen w-full bg-zinc-50 px-2 md:px-8">
                ...
                {!exibindoFormulario && (
                  <button
                      className="rounded-full bg-indigo-600 hover:bg-indigo-800 text-white px-6 sm:px-8 py-2.5 sm:py-3 font-bold text-base sm:text-lg shadow w-full sm:w-fit self-end mb-2 transition-colors duration-200"
                    onClick={() => {
                ...
                {!exibindoFormulario && (
                  <div className="flex justify-center mt-4 gap-2 w-full overflow-x-auto">
                    <div className="flex flex-col xs:flex-row items-center gap-2 w-full max-w-md">
                      <button
                        className="rounded-full bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-4 py-2 font-bold disabled:opacity-50 transition-colors duration-200 w-full xs:w-auto text-base xs:text-sm"
                        onClick={() => setPagina(p => Math.max(1, p - 1))}
                        disabled={pagina === 1}
                      >
                        Anterior
                      </button>
                      <span className="px-2 text-indigo-900 font-semibold text-center min-w-[110px] text-base xs:text-sm w-full xs:w-auto">
